...
export class AudioManager {
  ...
  updateVelocity(velocity) {
    ...
    try {
      ...
      if (this.sounds.ambient) {
        const ambientVol = CONFIG.audio.ambientVolume * (1 - normalizedSpeed * 0.5);
  ...
  muffle(enable) {
    ...
    ['ambient', 'deepSpace', 'blackholeHum', 'asteroidRumble'].forEach(soundName => {
      const sound = this.sounds[soundName];
      if (sound) {
        const currentVol = this.getDefaultVolume(soundName) * this.masterVolume;
