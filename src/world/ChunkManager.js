...
export class ChunkManager {
  ...
  update(cameraPosition) {
    ...
    if (
      currentChunkX !== this.lastCameraChunk.x ||
  ...
  generateChunk(chunkX, chunkY, chunkZ) {
    ...
    const nebulaNoise = this.simplex3d(
      chunkX * 0.1,
      chunkY * 0.1,
      chunkZ * 0.1
    );
    const voidNoise = this.simplex3d(
      chunkX * 0.05,
      chunkY * 0.05,
      chunkZ * 0.05
    );
      ...
      chunkData.features.push({
        ...
        position: {
          x:
            ...
            rng.range(-this.chunkSize / 2, this.chunkSize / 2),
          y:
    ...
    if (rng.chance(CONFIG.encounters.shipChance)) {
      chunkData.features.push({
        ...
        startPosition: {
          x:
            chunkX * this.chunkSize +
            rng.range(-this.chunkSize, this.chunkSize),
          y:
            chunkY * this.chunkSize +
            rng.range(-this.chunkSize, this.chunkSize),
          z:
