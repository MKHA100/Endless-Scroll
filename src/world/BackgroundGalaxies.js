import { Sprite, SpriteMaterial, TextureLoader, Group, CanvasTexture } from 'three';
...
export class BackgroundGalaxies {
  ...
  async init() {
    ...
    for (let i = 0; i < galaxyCount; i++) {
      ...
      // Position at extreme distance
      const distance = 6000 + rng.random() * 2000;
      const angle = rng.random() * Math.PI * 2;
      const elevation = (rng.random() - 0.5) * Math.PI * 0.5;
      
      sprite.position.x = Math.cos(angle) * Math.cos(elevation) * distance;
      sprite.position.y = Math.sin(elevation) * distance;
      sprite.position.z = Math.sin(angle) * Math.cos(elevation) * distance;
      ...
      // Rotation
      sprite.rotation = rng.random() * Math.PI * 2;
  ...
  update(deltaTime) {
    ...
    this.galaxies.forEach(galaxy => {
      galaxy.sprite.rotation += galaxy.rotationSpeed * deltaTime * 60;
    });
  }
