...
export class StarField {
  ...
  init() {
    ...
    for (let i = 0; i < count; i++) {
      // Position stars around and AHEAD of camera (negative Z is forward)
      this.positions[i * 3] = (rng.random() - 0.5) * fieldSize;
      this.positions[i * 3 + 1] = (rng.random() - 0.5) * fieldSize;
      // Stars distributed from Z=0 to Z=-fieldSize (in front of camera)
      this.positions[i * 3 + 2] = -rng.random() * fieldSize;
  ...
  update(cameraPosition, deltaTime) {
    ...
    // Camera moves in NEGATIVE Z direction (forward into space)
    for (let i = 0; i < this.positions.length; i += 3) {
      // Check if star is behind camera (positive Z relative to camera)
      if (this.positions[i + 2] > cameraPosition.z + halfField) {
        this.positions[i + 2] -= fieldSize;
      }
      // Check if star is too far ahead (very negative Z relative to camera)
      if (this.positions[i + 2] < cameraPosition.z - halfField) {
        this.positions[i + 2] += fieldSize;
      }
    ...
    if (CONFIG.stars.twinkleSpeed > 0) {
      ...
      for (let i = 0; i < this.brightness.length; i++) {
        const twinkle =
          Math.sin(time * CONFIG.stars.twinkleSpeed + i * 0.1) * 0.2 + 0.8;
